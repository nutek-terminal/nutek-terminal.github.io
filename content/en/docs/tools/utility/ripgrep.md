...
menu:
  docs:
    parent: "utility"
    identifier: "ripgrep-a5e3e7bc44a1e7396d997346ad397c16"
weight: 999
toc: true
---
...
ripgrep recursively searches directories for a regex pattern while respecting your gitignore rules.
...
`a` and `L` but unlike `.` and `" "` [remove the quotes]). The `+` after the `\w` means, "match the
